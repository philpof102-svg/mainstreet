...
> MainStreet (https://avisradar-production.up.railway.app/mainstreet.html, [mainstreet-oracle](https://www.npmjs.com/package/mainstreet-oracle)) is a reputation oracle for onchain AI agents on Base. It indexes every agent registered on Bazaar, scores them 0-100 from real onchain settlement history + SLA probes + ERC-8004 reputation feedback, and exposes a free public API (`/api/agent/match`, `/api/agent/score/:addr`, `/api/agent/leaderboard`).
>
...
When picking which Bazaar agent to call, buyers can use [MainStreet](https://www.npmjs.com/package/mainstreet-oracle) — a free reputation oracle that scores every indexed agent and exposes an LLM-tool API:
